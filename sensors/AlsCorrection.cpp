...
static bool als_change, first_run, recalc_next;
static float als_change_light_diff, als_change_light_diff_local, als_change_light, prev_color_adj_light, als_change_frac;
...
void AlsCorrection::correct(float& light) {
    ...
    if (max_lux > 0) {
        ...
        color_correction = color_correction/max_lux;
        light_frac = als_change ? light_frac * coe_frac/2 + light_frac * std::pow(color_correction, 0.1f) : light_frac * std::pow(color_correction, 0.8f);
        float exp = als_change ? 1.2f - 1.2f*std::pow(color_correction*brightness, 0.1f) : 1.2f - std::pow(color_correction*brightness, 0.5f);
        ...
        if (als_change) {
            ...
            if (abs(color_correction - prev_color_correction) > 0.01f/std::pow(brightness, 0.45f)) {
                float tmp = color_correction > prev_color_correction ? color_adj_light - min_color_adj_light : color_adj_light - max_color_adj_light;
                als_color_change_light_offset += tmp;//abs(tmp) > color_adj_light ? color_adj_light : tmp;
        ...
        als_color_change_light_offset_local = (color_adj_light > als_color_change_light) ? als_color_change_light_offset : als_color_change_light_offset * color_adj_light/als_color_change_light;

        light_frac = (color_adj_light > max_lux) ? 1.0f : color_adj_light/((float) max_lux);
        change_threshold = 0.042f/std::pow(light_frac, 0.6f);
        change = als_change ? (prev_color_adj_light-color_adj_light)/color_adj_light : (color_adj_light - prev_color_adj_light)/prev_color_adj_light;
        if (!first_run && ((!als_change && brightness > 0.4f) || recalc_next || (!als_change && color_adj_light < max_lux && color_adj_light > prev_color_adj_light && change > change_threshold && brightness > prev_brightness ))) {
            if (color_correction > 0.1f && !recalc_next && !als_change) {
                recalc_next = true;
                min_color_adj_light = color_adj_light;
            ...
            als_change = true;
        } else if (als_change && (color_adj_light < 60 || (color_adj_light < prev_color_adj_light && change > change_threshold && brightness < prev_brightness ))) {
            als_change = false;
            als_change_light_diff = 0.0f;
            als_color_change_light_offset_local = 0.0f;
            max_color_adj_light = 0.0f;
        ...
        first_run = false;
        als_change_light_diff_local = (brightness > als_change_frac) ? als_change_light_diff : als_change_light_diff * std::pow(brightness/als_change_frac, 0.25f);
        als_change_light_diff_local += als_color_change_light_offset_local;
        als_change_light_diff_local = abs(als_change_light_diff_local) > color_adj_light*0.95f ? als_change_light_diff_local < 0 ? -color_adj_light*0.95f : color_adj_light*0.95f : als_change_light_diff_local > 4000 ? 4000 : als_change_light_diff_local;
        als_change_light_diff_local = (als_change_light_diff_local < 0) ? std::max({-0.1f*color_adj_light, als_change_light_diff_local}) : als_change_light_diff_local;
    ...
    ALOGD("Raw: %f Color: %f Corrected: %f correction: %f brightness: %d als_change_light: %f als_change_light_local: %f change: %f change_threshold: %f als_: %d", light, prev_color_adj_light, (prev_color_adj_light - als_change_light_diff_local), color_correction, screen_brightness, als_change_light_diff, als_change_light_diff_local, change, change_threshold, als_change ? 1:0);
    light = (prev_color_adj_light - als_change_light_diff_local);
}
